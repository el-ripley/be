...
OUTFIT_SCHEMA = {"type": "object", "properties": {"weather_summary": {"type": "string"}}, "required": ["weather_summary"]}
...
def run_anthropic() -> None:
    ...
    tools = [
        ...
        {"name": "suggest_outfit", "description": "Suggest outfit based on weather", "input_schema": OUTFIT_SCHEMA},
    ]
    ...
    for turn in range(max_turns):
        ...
        turn_log.append({
            ...
            "content_block_types": [getattr(b, "type", getattr(getattr(b, "model_dump", lambda: {})(), "type", None)) for b in (response.content or [])],
        })
        ...
        for tc in tool_calls:
            if tc["name"] == "get_weather":
                results.append({"type": "tool_result", "tool_use_id": tc["id"], "content": "25°C, sunny"})
            else:
                results.append({"type": "tool_result", "tool_use_id": tc["id"], "content": "Light shirt and jeans."})
        messages.append({"role": "user", "content": results})

    save_evidence(
        test_name=TEST_NAME,
        provider="anthropic",
        request_data={"method": "messages.create (loop)", "params": {"model": model, "tools": tools, "initial_messages": [{"role": "user", "content": PROMPT}]}},
        raw_response={
            "turn_log": turn_log,
        ...
        mapping={
            ...
            "differences": ["Anthropic: content array with tool_use; tool_result in user message."],
    ...
    print("Anthropic: OK", "-", len(turn_log), "turns", "-", "final len(messages):", len(messages))
...
def run_gemini() -> None:
    ...
    tool = types.Tool(
        function_declarations=[
            ...
            types.FunctionDeclaration(name="suggest_outfit", description="Suggest outfit based on weather", parameters=OUTFIT_SCHEMA),
        ]
    )
    ...
    for turn in range(1, max_turns + 1):
        ...
        tool_parts = [types.Part.from_function_response(name=fc["name"], response={"result": "25°C, sunny" if fc["name"] == "get_weather" else "Light shirt and jeans."}) for fc in function_calls]
        contents.append(types.Content(role="tool", parts=tool_parts))

    save_evidence(
        test_name=TEST_NAME,
        provider="gemini",
        request_data={"method": "models.generate_content (loop)", "params": {"model": model, "tools": [tool], "initial_prompt": PROMPT}},
        raw_response={
            "turn_log": turn_log,
            "final_text_preview": final_text[:300],
        },
        ...
        mapping={
            ...
            "differences": ["Gemini chat keeps history; no explicit messages array to maintain."],
        ...
        model_in_response=getattr(response, "model_version", None) if response else raw.get("model_version") if isinstance(raw, dict) else None,
    )
